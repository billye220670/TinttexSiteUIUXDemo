...
> 但**所有后端契约（数据类型、字段格式、认证方式、端点）尚未确定**，需要后端拍板。

---

## 0. 给接手的 AI Agent 的工作指令（请先读这一段）

你（AI Agent）正在协助开发者为这个纯前端落地页对接后端。请严格按以下流程工作：

1. **先通读全文**，理解前端现状（§1–§3）、对接锚点（§3）、以及必须遵守的前端约束与陷阱（§6）。
2. **【强制】不要直接写代码。** 先带开发者逐条过 **§5「待澄清清单」**，把每一条的答案问清楚并记录下来。
   这些是前端无法单方面决定、必须由后端确认的**数据类型 / 字段格式 / 认证方案 / 端点契约**。
   **禁止假设默认值、禁止跳过。** 澄清不完整就不要开工。
3. 澄清完成后，按 **§3 的锚点** + **§4 的建议接线方式**开始实现，契约以 §5 的澄清结果为准。
4. 实现全程遵守 **§6「前端约束与陷阱」**，不要破坏已有的纯前端交互（i18n、弹窗、hero 滚动动画、照片墙）。
5. 当需要新增前端目前不存在的 UI（如**错误提示条、加载态、扫码登录状态**）时，**先与开发者确认**放置位置、样式与文案；
   若文案要多语言，需按 §6.1 的方式接入 i18n 字典，不要硬编码单语言字符串。
6. 完成后对照 **§8 完成定义（DoD）**自检。
...
## 5. 待澄清清单（**Agent 必须先与开发者逐条确认，填完再开工**）

> 说明：下列每一项前端都无法替你决定。请 Agent 逐条向开发者提问，把答案写进「答复」栏，再据此实现。
